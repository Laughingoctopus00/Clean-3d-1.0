...
void CSMain(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    float4 color = ScreenTexture.SampleLevel(LinearSampler, uv, 0);
    ...
    if (edge_depth_influence > 0.0)
    {
        ...
        edge = saturate(edge * edge_depth_influence * 2.0); // Amplify edge contribution
    }
    
    // Combine depth and edge information (increased edge weight)
    float edge_blend_factor = 0.75; // Configurable blend factor
    DepthTexture[pixelCoord] = lerp(depth, edge, edge_blend_factor);
}